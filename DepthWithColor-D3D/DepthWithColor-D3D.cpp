...
/// Constructor
/// </summary>
CDepthWithColorD3D::CDepthWithColorD3D(ID3D11Device* m_pd3dDevice, ID3D11DeviceContext* m_pImmediateContext)
{
...
/// </summary>
/// <returns>S_OK for success, or failure code</returns>
HRESULT CDepthWithColorD3D::Render(ID3D11DeviceContext* immediate, const XMMATRIX& viewProjection)
{
    if (m_bPaused)
    {
        return S_OK;
	...
	immediate->ClearRenderTargetView(m_pRenderTargetView, ClearColor);

    // Clear the depth buffer to 1.0 (max depth)
	immediate->ClearDepthStencilView(m_pDepthStencilView, D3D11_CLEAR_DEPTH, 1.0f, 0);
	*/
    ...
    cb.Projection = XMMatrixTranspose(viewProjection);
    cb.XYScale = XMFLOAT4(m_xyScale, -m_xyScale, 0.f, 0.f); 
    immediate->UpdateSubresource(m_pCBChangesEveryFrame, 0, NULL, &cb, 0, 0);

    // Set up shaders
	ID3D11VertexShader* oldShader = 0; 
	ID3D11GeometryShader* oldGS = 0; 
	ID3D11PixelShader* oldPS = 0; 

	immediate->VSGetShader(&oldShader, 0, 0);
	immediate->GSGetShader(&oldGS, 0, 0); 
	immediate->PSGetShader(&oldPS, 0, 0); 
	
	immediate->VSSetShader(m_pVertexShader, NULL, 0);

    immediate->GSSetShader(m_pGeometryShader, NULL, 0);
    immediate->GSSetConstantBuffers(0, 1, &m_pCBChangesEveryFrame);
    immediate->GSSetShaderResources(0, 1, &m_pDepthTextureRV);
    immediate->GSSetShaderResources(1, 1, &m_pColorTextureRV);
    immediate->GSSetSamplers(0, 1, &m_pColorSampler);

    immediate->PSSetShader(m_pPixelShader, NULL, 0);

    // Draw the scene
    immediate->Draw(m_depthWidth * m_depthHeight * 4, 0);


	immediate->VSSetShader(oldShader, NULL, 0);
	immediate->GSSetShader(oldGS, NULL, 0);
	immediate->PSSetShader(oldPS, NULL, 0);

    // Present our back buffer to our front buffer
	return 0;// m_pSwapChain->Present(0, 0);
}